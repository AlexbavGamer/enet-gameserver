        ...
        switch (packet.type) {
            ...
            case PacketType::PLAYER_MOVE: {
                if (players_.count(packet.peer_id)) {
                    ...
                    // Parse movimento
                    std::string json_str(packet.data.begin(), packet.data.end());
                    nlohmann::json move_data = nlohmann::json::parse(json_str);
                    
                    Vector3 old_pos = player->getPosition();
                    Vector3 new_pos{
                        move_data["x"].get<float>(),
                        move_data["y"].get<float>(),
                        move_data["z"].get<float>()
                    };
                    
                    // Anti-cheat validation
                    if (Config::getInstance().isAntiCheatEnabled()) {
                        if (!anti_cheat_->validateMovement(packet.peer_id, 
                            old_pos.x, old_pos.z, new_pos.x, new_pos.z, 0.016f)) {
                            
                            if (anti_cheat_->shouldBanPlayer(packet.peer_id)) {
                                Logger::error("Banning player " + std::to_string(packet.peer_id) + 
                                            " for cheating");
            ...
            case PacketType::PLAYER_ACTION:
                if (anti_cheat_->validatePlayerAction(packet.peer_id, "action")) {
                    lua_manager_->callFunction("handle_player_action", packet.peer_id, packet.data);
                }
                break;
                
            case PacketType::CHAT_MESSAGE:
                lua_manager_->callFunction("handle_chat_message", packet.peer_id, packet.data);
                break;
